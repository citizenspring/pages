...
async function loadHostConfigFromGoogleDocId(host) {
  ...
  if (metadata.icon && metadata.icon.src) {
    ...
    icon = { src: host.favicon || host.icon || "/favicon.ico" };
  }
  metadata.sitemap["/"] = {
...
export async function getHostConfig(hostname) {
  ...
  if (hostConfig.sitemap && !hostConfig.googleDocId) {
    const newHostConfig = {};
    const sitemap = {};
    ...
    for (let i = 0; i < slugs.length; i++) {
      ...
      if (slug === "index") {
        newHostConfig.icon = {
          src: pageInfo.svgicon || pageInfo.icon || pageInfo.favicon,
        };
        newHostConfig.title = pageInfo.title;
        newHostConfig.googleDocId = pageInfo.googleDocId;
      }
    }
    newHostConfig.hostname = hostname;
    newHostConfig.sitemap = sitemap;
    configCache.set(hostname, newHostConfig);

...
import { isGoogleDocId, getPageMetadataByGoogleDocId } from "./lib";
...
function convertMarkdownToHTML(markdown) {
  ...
  return `<html><body><div id="contents"><div>${html}</div></div></body></html>`;
}
...
function cleanHTML(html) {
  ...
  return (
    html
      ...
      .replace(
        /<a [^>]*>https?:\/\/calendar.google.com\/calendar(?:\/u\/0)?\/embed\?([^<]+)<\/a>/gi,
        (match, p1, p2, p3, p4) => `<GoogleCalendar params="${p1}" />`
      )
      ...
      // Twitter embed
      .replace(
        /<a [^>]*>(https?:\/\/(www\.)?(?:twitter|x)\.com\/(.){1,15}\/status(es)?\/([0-9]+)[^<]*)<\/a>/gi,
      ...
      .replace(
        ...
        (match, border, classes, href, label) => {
          return `<Button href="${removeGoogleURLRedirect(href)}" ${
            border && border.length == 2 ? 'primary="true"' : ""
          }>${label}</Button>`;
        }
      )
      // <a href="">[button]</a>
...
export function extractMetadata(html) {
  ...
  return { body, title, description, sitemap, redirections, icon };
}
...
const processHTML = (htmlText) => {
  ...
  try {
    ...
    console.log(
      "!!! processHTML > renderToString error",
      e,
      "reactTree:",
      reactTree
    );
...
export const downloadGoogleDoc = async (docid, format = "pdf") => {
  const exportUrl = `https://docs.google.com/feeds/download/documents/export/Export?id=${docid}&exportFormat=${format}`;
  const res = await fetch(exportUrl);
  if (res.status !== 200) {
    throw new Error(
      `Error downloading google doc ${docid}: ${res.status} ${res.statusText}`
    );
  }
  if (format === "markdown") {
    return res.text();
...
export async function fetchGoogleDoc(docid, format = "html") {
  if (!isGoogleDocId(docid)) {
    throw new Error(`invalid_googledocid (${docid})`);
  }
  if (format === "markdown") {
    const markdown = await downloadGoogleDoc(docid, "markdown");
  ...
  try {
    ...
    console.log(">>> downloading markdown version of google doc", docid);
    const markdown = await downloadGoogleDoc(docid, "markdown");
    if (markdown) {
...
export async function getHTMLFromGoogleDocId(docid, hostConfig) {
  ...
  let htmlText;
  if (hostConfig.format === "markdown") {
    const markdown = await downloadGoogleDoc(docid, "markdown");
    htmlText = convertMarkdownToHTML(markdown);
    // googleDocCache.set(docid, doc);
    // doc = { body: html };
  } else {
    htmlText = await fetchGoogleDoc(docid);
  }
  const doc = processHTML(htmlText);

  if (!doc) {
    console.log(">>> doc is null", docid, "htmlText:", htmlText);
    throw new Error(`Unable to process google doc id ${docid}`);
  }
